...
const socials = [
  { name: "Telegram", href: "https://t.me/cv_offshore_mailer" }
];
...
function HowItWorks() {
  const steps = [
    "Choose OFFSHORE plan",
    "Share your CV link and basic details",
    "We mail it to verified offshore companies",
    "You receive replies directly"
  ];
  return (
    <section id="how" className="bg-white">
      <div className="container py-14">
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {steps.map(s => (
            <Card key={s}><CardHeader><CardTitle className="text-lg">{s}</CardTitle></CardHeader></Card>
          ))}
        </div>
      </div>
    </section>
    ...
    <section id="benefits" className="border-y bg-white">
      <div className="container py-14">
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {list.map(b => (
            <Card key={b.t}><CardHeader><CardTitle className="text-lg">{b.t}</CardTitle><CardDescription>{b.d}</CardDescription></CardHeader></Card>
          ))}
        </div>
      </div>
    </section>
          ...
          <Card className="rounded-2xl">
            ...
            <CardContent>
              <ul className="space-y-2 text-sm">
                {PLAN.features.map(f => (<li key={f} className="flex items-start gap-2">• {f}</li>))}
              </ul>
            </CardContent>
...
function Testimonials() {
  const items = [
    { n: "Oleksiy — Master", t: "Got several offers within a week. Report was clear." },
    { n: "Serhii — Chief Officer", t: "Fast delivery and real replies to my inbox." },
  ];
  return (
    <section id="testimonials" className="bg-white">
      <div className="container py-14">
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-2">
          {items.map(x => (
            <Card key={x.n}><CardHeader><CardTitle className="text-base">{x.n}</CardTitle></CardHeader><CardContent><p className="text-sm text-neutral-600">{x.t}</p></CardContent></Card>
          ))}
        </div>
      </div>
    </section>
    ...
    <section id="order" className="bg-white">
      <div className="container py-14 max-w-3xl">
        ...
        <div className="mt-6 grid gap-4">
          ...
          <p className="text-xs text-neutral-500">By sending, you agree to our data processing. We don’t store your documents — they are sent directly to employers.</p>
        </div>
      </div>
    </section>
  );
